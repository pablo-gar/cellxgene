...
function _histogramContinuousBy(column: any, bins: any, min: any, max: any, by: any) {
  const byMap = new Map();
  if (!column || !by) {
    return byMap;
  }
...
*/
export function histogramContinuous(column: any, bins = 40, domain = [0, 1], by: any) {
  if (by && isTypedArray(by)) {
    throw new Error("Group by column must be categorical");
  }
...
*/
export function hashContinuous(column: any, bins = "", domain = [0, 0], by: any) {
  const [min, max] = domain;
  if (by) {
